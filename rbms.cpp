...
void rbms::rbms_read(CANMessage &msg, int64_t *rotation,short *speed) {//motorからの受信データを変換する関数
            _r = (msg.data[0] << 8) | (msg.data[1] & 0xff);//2byteに分割されているdataを結合

            int64_t high_bits = _rotation & 0xFFFFFFFFFFFFE000;
            int64_t new_position_candidate = high_bits | (uint16_t)(_r & 0x1FFF);
            int64_t diff = new_position_candidate - _rotation;

            if (diff > 4096) {
                new_position_candidate -= 8192;
            } else if (diff < -4096) {
                new_position_candidate += 8192;
            }
            _rotation = new_position_candidate; // 次回のために自身の状態を更新
            *rotation = (float)new_position_candidate / 8192.0 * 360.0;
            ...
            _motor_type ? _speed /= 19 : _speed /= 36;
            *speed=_speed;
...
void rbms::spd_control(int* set_speed,int* motor){//速度制御用関数
    int64_t rotation[_motor_num];
    short speed[_motor_num];
    ...
    while(1){
        for(int id=0;id<_motor_num;id++){
            if(_msg.id==0x201+id){//esc idごとに受信データ割り振り
                ...
                if(_motor_type[id]){
                    motor[id] = (int)pid(chrono::duration<float>(tm[id].elapsed_time()).count(),speed[id],set_speed[id],&delta_rpm_pre[id],&ie[id]);
                }else{
                    motor[id] = (int)pid(chrono::duration<float>(tm[id].elapsed_time()).count(),speed[id],set_speed[id],&delta_rpm_pre[id],&ie[id],15,6);
                }
